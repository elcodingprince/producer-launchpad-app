...
export function parseExecutedAgreementLicense(
  resolvedLicenseJson?: string | null,
): ExecutedAgreementResolvedLicense | null {
  if (!resolvedLicenseJson) return null;

  try {
    return JSON.parse(resolvedLicenseJson) as ExecutedAgreementResolvedLicense;
  } catch {
    return null;
  }
}

type ShopifyAgreementContext = {
  productTitle: string;
...
): Promise<ShopifyAgreementContext> {
  const query = `#graphql
...
export async function buildExecutedAgreementSnapshot(input: {
  shop: string;
  order: Pick<
    Order,
    | "id"
  ...
  const [licensorMetaobject, producerMetaobject, shopifyData] =
    await Promise.all([
      ...
      fetchAgreementDocumentData(admin, {
        ...
        lineItemId: input.orderItem.shopifyLineId,
  ...
  if (!licensorMetaobject) {
    throw new Error(
      "A default licensor must be configured before executed agreements can be generated.",
    );
  }
  ...
  const primaryProducerName =
    getFieldValue((producerMetaobject as MetaobjectLike)?.fields, "name") || "";
  ...
  const resolvedStemsPolicy = resolveOfferStemsPolicy(
  ...
  const license = {
    ...
    licenseName:
      getFieldValue(licenseFields, "license_name") ||
      input.orderItem.licenseName,
  ...
  const agreement = await renderAgreementPreview({
    ...
    context: {
      ...
      purchaseDate: input.order.createdAt.toLocaleDateString("en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      }),
      beatTitle: input.orderItem.beatTitle || shopifyData.productTitle,
      ...
      buyerIp: input.order.browserIp?.trim() || shopifyData.clientIp || "",
      userAgent: input.order.userAgent?.trim() || "",
      stemsIncludedInOrder: stemsIncludedInFinalOrder,
    },
  });

  const renderedHtml = agreement.html;
  const htmlHash = hashValue(renderedHtml);
  ...
  try {
    ...
    return {
      ...
      templateMetaobjectId: shopifyData.licenseReference.id,
      ...
      beatOfferSnapshotJson: JSON.stringify({
        variantId: input.orderItem.variantId,
        stemsAddonEnabled: shopifyData.stemsAddonEnabled,
        resolvedStemsPolicy,
        stemsIncludedInOrder: stemsIncludedInFinalOrder,
      }),
      stemsIncludedInOrder: stemsIncludedInFinalOrder,
      licensorSnapshotJson: JSON.stringify({
        licensor,
        producerAliases,
      }),
      renderedHtml,
      ...
      buyerIp: input.order.browserIp?.trim() || shopifyData.clientIp || null,
      userAgent: input.order.userAgent?.trim() || null,
      purchasedAt: input.order.createdAt,
    };
  } catch (error) {
    return {
      shop: input.shop,
      ...
      pdfError:
        error instanceof Error
          ? error.message
          : "Failed to generate executed agreement PDF",
      buyerEmail: input.customerEmail,
      buyerIp: input.order.browserIp?.trim() || shopifyData.clientIp || null,
      userAgent: input.order.userAgent?.trim() || null,
      purchasedAt: input.order.createdAt,
